...
import { teamLogosPremier } from "../assets/teamLogosPremier";
...
const getPositionColor = (pos: number, total: number): string => {
  ...
  if (pos >= 2 && pos <= 5) return "#1E90FF";
  if (pos === 6) return "#FF6C41";
  if (pos === 7) return "#4CAF50";
  if (pos > total - 3) return "#FF0000";
  return "transparent";
};

const TablaClasificacionPremier: React.FC = () => {
  ...
  return (
    <div className="menu-screen flex flex-col min-h-screen">
      ...
      {/* CONTENT */}
      <main className="flex-grow bg-[#272e3f] text-[#c0e1ec] font-sans flex justify-center items-start content-spacing py-6">
        <div className="w-[96%] max-w-sm px-2">
          <header className="flex items-center justify-between mb-4">
            <h1 className="text-xl font-bold">Clasificación Premier League</h1>
            <img src={leagueLogos.premier} alt="Logo Premier League" style={{ height: '80px' }} />
          </header>
    ...
    {equipo.RL}
                ...
                <div className="w-[32%] flex items-center">
                  <div style={{ width:44, height:44, overflow:'hidden', marginRight:6, display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <img src={teamLogosPremier[equipo.Equipo]||'/logos/placeholder.png'} alt={`${equipo.Equipo} logo`} style={{ width:40, height:40 }} onError={e=>{(e.target as HTMLImageElement).src='/logos/placeholder.png';}} />
                  </div>
                  <span className="truncate cursor-pointer" onClick={() => handleNavigateToStats(equipo.Equipo)}>
                    {equipo.Equipo}
                  </span>
                </div>
...
export default TablaClasificacionPremier;