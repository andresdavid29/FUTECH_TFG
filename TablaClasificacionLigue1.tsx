...
import { teamLogosLigue1 } from "../assets/teamLogosLigue1";
...
const getPositionColor = (pos: number, total: number): string => {
  ...
  if (pos >= 5 && pos <= 6) return "#FF6C41";
  if (pos === 7) return "#4CAF50";
  if (pos > total - 3) return "#FF0000";
  return "transparent";
};

const TablaClasificacionLigue1: React.FC = () => {
  ...
  const handleNavigateToStats = (eq: string) =>
    navigate(`/estadisticas/${encodeURIComponent(eq)}`);
      ...
      <main className="flex-grow bg-[#272e3f] text-[#c0e1ec] font-sans flex justify-center items-start content-spacing">
        <div className="w-[96%] max-w-sm px-2 py-6">
          <header className="flex items-center justify-between mb-4">
            <h1 className="text-xl font-bold">Clasificación Ligue 1</h1>
            <img src={leagueLogos.ligue_1} alt="Logo Ligue 1" style={{ height: '80px' }} />
          </header>
    ...
    {equipo.RL}
                ...
                <div className="w-[32%] flex items-center">
                  <div style={{ width:44, height:44, overflow:'hidden', marginRight:6, display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <img src={teamLogosLigue1[equipo.Equipo] || '/logos/placeholder.png'} alt={`${equipo.Equipo} logo`} style={{ width:40, height:40 }} onError={e => { (e.target as HTMLImageElement).src='/logos/placeholder.png'; }} />
                  </div>
...
export default TablaClasificacionLigue1;
