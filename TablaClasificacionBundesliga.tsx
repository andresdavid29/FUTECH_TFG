...
import { teamLogosBundesliga } from "../assets/teamLogosBundesliga";
...
const getPositionColor = (pos: number, total: number): string => {
  ...
  if (pos === 6) return "#4CAF50";
    if (pos === 9) return "#FF6C41";
  if (pos > total - 3) return "#FF0000";
  return "transparent";
};

const TablaClasificacionBundesliga: React.FC = () => {
  ...
  useEffect(() => {
    const unsubAuth = onAuthStateChanged(auth, (fbUser) => {
      setUser(fbUser);
      setAuthChecked(true);
      if (!fbUser) setFavoritos([]);
      else {
        const favsRef = collection(db, "users", fbUser.uid, "favorites");
  ...
  const handleNavigateToStats = (eq: string) => navigate(`/estadisticas/${encodeURIComponent(eq)}`);
  ...
  return (
    <div className="menu-screen flex flex-col min-h-screen">
      ...
      {/* CONTENT */}
      <main className="flex-grow bg-[#272e3f] text-[#c0e1ec] font-sans flex justify-center items-start content-spacing">
        <div className="w-[96%] max-w-sm px-2 py-6">
          <header className="flex items-center justify-between mb-4">
            <h1 className="text-xl font-bold">Clasificación Bundesliga</h1>
            <img src={leagueLogos.bundesliga} alt="Logo Bundesliga" style={{ height: '80px' }} />
          </header>
...
<div className="w-[10%] pl-2 flex items-center" style={{ position: 'relative' }}>
  ...
  />
  <span style={{ marginLeft: 8, position: 'relative', zIndex: 1 }}>
    {equipo.RL}
  </span>
</div>
                <div className="w-[32%] flex items-center">
                  <div style={{ width:44, height:44, overflow:'hidden', marginRight:6, display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <img src={teamLogosBundesliga[equipo.Equipo] || '/logos/placeholder.png'} alt={`${equipo.Equipo} logo`} style={{ width:40, height:40 }} onError={e => { (e.target as HTMLImageElement).src='/logos/placeholder.png'; }} />
                  </div>
                  <span className="truncate cursor-pointer" onClick={() => handleNavigateToStats(equipo.Equipo)}>{equipo.Equipo}</span>
                </div>
                ...
                <div className="w-[14%] text-center pr-1 flex justify-center">
                  <Heart className="w-5 h-5 cursor-pointer transition-colors" strokeWidth={2} stroke={isFav ? undefined : '#9ca3af'} fill={isFav ? '#ef4444':'none'} onClick={() => toggleFavorito(equipo.Equipo)} />
                </div>
              </div>
            );
          })}
...
export default TablaClasificacionBundesliga;