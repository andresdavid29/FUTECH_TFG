...
type Equipo = { RL: number; Equipo: string; Pts: number; GF: number; GC: number; DG: number; };

const getPositionColor = (pos: number, total: number): string => {
  if (pos === 1) return "#FFD700";
  if (pos >= 2 && pos <= 5) return "#1E90FF";
  if (pos >= 6 && pos <= 7) return "#FF6C41";
  if (pos === 8) return "#4CAF50";
  if (pos > total - 3) return "#FF0000";
  return "transparent";
};

const TablaClasificacionLaLiga: React.FC = () => {
      ...
      <main className="flex-grow content-spacing flex justify-center items-start py-6">
        <div className="w-[96%] max-w-sm px-2">
          <header className="flex items-center justify-between mb-4">
            <h1 className="text-xl font-bold">Clasificación La Liga</h1>
            <img src={leagueLogos.la_liga} alt="Logo La Liga" style={{ height: '80px' }} />
          </header>
          ...
          {equipos.map((equipo, idx) => {
            ...
            return (
              <div
                key={equipo.Equipo}
                className={`${bg} flex items-center text-xs py-1`}
    ...
    {equipo.RL}
                ...
                <div className="w-[32%] flex items-center">
                  <div style={{ width:44, height:44, overflow:'hidden', marginRight:6, display:'flex', alignItems:'center', justifyContent:'center' }}>
                    <img
                      src={teamLogos[equipo.Equipo] || '/logos/placeholder.png'}
                      alt={`${equipo.Equipo} logo`}
...
export default TablaClasificacionLaLiga;
